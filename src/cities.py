...
for country_index in range(country_count):
    country_str = input("Please describe the " +
                        str(country_index + 1) + " country:")
...
for index_cities in range(cities_count):
    lst_cities.append(input("Please enter the" +
                            str(index_cities + 1) + " city"))
for city in lst_cities:
    print(list(country for (country, cities) in country_cities.items()
               if city in cities)[0])
