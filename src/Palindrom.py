...
while lenstr2 > 1 or number != 0:  # сравниваем попарно первую и последнюю цифру и обрезаем число с двух сторон
    first = number2 // (10 ** (lenstr2 - 1))  # первая цифра
    last = number2 % 10  # последняя цифра
    if first != last:  # если цифры не равны прекратить цикл
        print("Обычное")
        break
    number = (number2 - last - first * (10 ** lenstr2)) // 10  # обрезаем с двух сторон
